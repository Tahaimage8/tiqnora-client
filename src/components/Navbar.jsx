...
const Navbar = () => {
  ...
  // Design only. Later this will come from real auth/session.
  ...
  return (
    <nav className="sticky top-0 z-50 w-full border-b border-white/10 bg-[#080C16]/80 backdrop-blur-2xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
        {/* Logo - do not wrap with Link again because Logo already has Link */}
          ...
          {!isLoggedIn ? (
            <>
              ...
              <button
                ...
                className="rounded-full border border-white/10 px-4 py-2.5 text-xs font-semibold text-slate-400 transition hover:border-purple-400/40 hover:text-white"
              >
                Preview User
              </button>
            </>
              ...
              {dropdownOpen && (
                <div className="absolute right-0 mt-3 w-64 overflow-hidden rounded-3xl border border-white/10 bg-[#101828]/95 shadow-2xl shadow-black/40 backdrop-blur-2xl">
                  ...
                  <div className="p-2">
                    ...
                    <Link
                      href={`/dashboard/${mockUser.role}/profile`}
                      onClick={() => setDropdownOpen(false)}
                      className="flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-medium text-slate-300 transition hover:bg-white/5 hover:text-white"
                    >
              ...
              {!isLoggedIn ? (
                <div className="grid gap-3">
                  ...
                  <button
                    ...
                    className="rounded-2xl border border-white/10 px-4 py-3 text-center text-xs font-semibold text-slate-400"
                  >
                    Preview User
                  </button>
                </div>
                ...
                <button
                  onClick={handleLogout}
                  className="w-full rounded-2xl border border-red-500/20 px-4 py-3 text-sm font-semibold text-red-400 transition hover:bg-red-500/10"
                >
                  Log Out
                </button>
              )}
